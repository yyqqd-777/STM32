...
/**
 * @function: void OLED_Printf(uint8_t x, uint8_t y, uint8_t size, uint8_t color, const char *format, ...)
 * @description: 在OLED上按照printf格式输出各种数据
 * @param {uint8_t} x 显示的起始横坐标 x:0~127
 * @param {uint8_t} y 显示的起始纵坐标 y:0~7
 * @param {uint8_t} size 字体大小，选择 16/12
 * @param {uint8_t} color 是否反相显示(1反相、0不反相)
 * @param {const char*} format 格式化字符串，类似printf用法
 * @param {...} 可变参数列表
 * @return {*}
 */
void OLED_Printf(uint8_t x, uint8_t y, uint8_t size, uint8_t color, const char *format, ...)
{
    ...
    OLED_ShowString(x, y, buffer, size, color);
}
